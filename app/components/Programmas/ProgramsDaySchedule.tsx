...
const schedule = [
  { time: "7:30 AM", activity: "Arrival & Morning Welcome", desc: "Children arrive, greet teachers and settle in with free play.", icon: <Sun size={26} />, color: "#f5a623" },
  { time: "8:00 AM", activity: "Circle Time", desc: "Songs, rhymes, calendar, weather and sharing circle.", icon: <Star size={26} />, color: "#e05c5c" },
  { time: "8:30 AM", activity: "Snack & Nutrition Break", desc: "Healthy snacks, hygiene habits and social dining skills.", icon: <Utensils size={26} />, color: "#5cb85c" },
  { time: "9:30 AM", activity: "Structured Learning", desc: "Age-appropriate lessons in language, numbers and concepts.", icon: <BookOpen size={26} />, color: "#00acc1" },
  { time: "10:00 AM", activity: "Art & Creative Play", desc: "Drawing, painting, craft and imaginative free expression.", icon: <Palette size={26} />, color: "#7e57c2" },
  { time: "10:45 AM", activity: "Music & Movement", desc: "Dance, rhythm, action songs and musical instruments.", icon: <Music size={26} />, color: "#f5a623" },
  { time: "11:15 AM", activity: "Outdoor & Nature Play", desc: "Physical activity, garden exploration and sensory play.", icon: <TreePine size={26} />, color: "#5cb85c" },
  { time: "12:00 PM", activity: "Computer & STEM", desc: "Age-appropriate digital learning and problem solving.", icon: <Monitor size={26} />, color: "#00acc1" },
  { time: "12:30 PM", activity: "Rest & Story Time", desc: "Relaxation, guided imagery and bedtime story reading.", icon: <Moon size={26} />, color: "#7b5ea7" },
];
...
export default function ProgramsDaySchedule() {
 ...
 useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting && !hasAnimated.current) {
          ...
          setTimeout(() => {
            // Line draws very slowly — 60ms intervals, 2% increments = ~3s total
            let width = 0;
            ...
            // Icons appear very slowly — 400ms apart each
  ...
  .icon-pop {
    animation: popIn 0.9s cubic-bezier(0.34, 1.2, 0.64, 1) forwards;
  }
  .icon-float {
...
`}</style>
      ...
      {/* Floating shapes */}
      <div className="absolute left-[3%] top-[20%] w-16 h-16 sun-float z-10 opacity-80 pointer-events-none hidden md:block">
      ...
      <div className="container-custom relative z-10">
        ...
        <div ref={sectionRef} className="relative px-4">
          ...
          <div className="absolute top-[27px] left-0 right-0 h-[3px] hidden lg:block overflow-hidden rounded-full" style={{ backgroundColor: "transparent" }}>
            <div
          ...
          {/* Icons row */}
          <div className="grid grid-cols-3 md:grid-cols-5 lg:grid-cols-9 gap-2 lg:gap-20">
